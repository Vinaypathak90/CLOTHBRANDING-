...
import { Save, Plus, Trash2, Edit3, Check, X, Layout, Image, List, ShieldAlert, CheckCircle, RefreshCw, Package, LogOut } from 'lucide-react';
...
function IdentityNavbarTab({ config, setConfig, newMenu, setNewMenu, onAddNavItem, onRemoveNavItem }) {
  ...
  return (
    <div className="w-full grid grid-cols-1 xl:grid-cols-2 gap-8 items-start block min-w-0">
      ...
      <div className="w-full bg-white border border-neutral-200 p-6 rounded-xl flex flex-col gap-6 text-left block">
        <h3 className="font-['Playfair_Display'] text-lg font-medium text-[#b5862a] border-b border-neutral-100 pb-3 block">Navigation Links Menu</h3>
        
        <div className="bg-[#f7f4ef]/60 p-4 rounded-xl border border-dashed border-neutral-200 flex flex-col gap-3 w-full block">
        ...
        <div className="flex flex-col gap-3 max-h-[450px] overflow-y-auto w-full block pr-1">
          {menuItems.map((menuItem, idx) => (
            <div key={idx} className="w-full bg-neutral-50 border border-neutral-200 p-4 rounded-xl flex justify-between items-center shadow-2xs">
              {editingNavIndex === idx ? (
  ...
  return (
    <div className="w-full grid grid-cols-1 xl:grid-cols-2 gap-8 items-start block min-w-0">
      ...
      <div className="w-full bg-white border border-neutral-200 p-6 rounded-xl flex flex-col gap-5 text-left block">
        ...
        <div className="flex flex-col gap-4 border-t border-neutral-100 pt-4 mt-2 w-full block">
      ...
      <div className="w-full bg-white border border-neutral-200 p-6 rounded-xl flex flex-col gap-6 text-left block">
        ...
        <div className="w-full border-t border-neutral-100 pt-5 mt-2 block">
          ...
          <div className="w-full grid grid-cols-1 sm:grid-cols-2 gap-4 max-h-[450px] overflow-y-auto p-1 block">
            {heroImages.map((imgUrl, idx) => (
              <div 
                key={idx} 
                className="bg-neutral-50 border border-neutral-200 p-3 rounded-xl flex flex-col gap-3 relative shadow-2xs min-w-0 block"
              >
...
function FeaturesGridTab({ config, setConfig, newFeature, setNewFeature, onAddFeatureItem, onRemoveFeatureItem }) {
  ...
  return (
    <div className="w-full grid grid-cols-1 xl:grid-cols-2 gap-8 items-start block min-w-0">
      ...
      <div className="w-full bg-white border border-neutral-200 p-6 rounded-xl text-left flex flex-col gap-4">
      ...
      <div className="flex flex-col gap-5 text-left min-w-0">
        ...
        <div className="flex flex-col gap-4 max-h-[500px] overflow-y-auto pr-1 w-full block">
          {features.map((item, idx) => (
            <div key={idx} className="w-full bg-white p-5 rounded-xl border border-neutral-200 flex gap-4 items-start justify-between shadow-xs hover:shadow-sm transition-all duration-300 text-left">
              ...
              {editingFeatureIndex === idx ? (
                ...
                <>
                  <div className="flex gap-4 items-start">
                    <span className="w-7 h-7 bg-[#b5862a] text-white text-xs font-bold flex items-center justify-center rounded-full flex-shrink-0 mt-0.5 shadow-2xs">{idx + 1}</span>
  ...
  return (
    <div className="w-full min-h-screen bg-[#f7f4ef] text-[#1a1a1a] font-['DM_Sans'] flex items-stretch overflow-x-hidden">
      ...
      <main className="flex-grow p-6 sm:p-10 lg:p-12 overflow-y-auto block text-left">
        <div className="max-w-[1400px] mx-auto w-full block">
          ...
          <div className="w-full border-b border-[#e8e2d8] pb-6 mb-8 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 select-none">
            <div>
              ...
              <h1 className="font-['Playfair_Display'] text-2xl sm:text-3xl font-normal text-neutral-800">
                {activeTab === 'identity' ? 'Identity & Navigation Matrix' : activeTab === 'hero' ? 'Hero Layout Architecture' : 'Features Structural Grid'}
              </h1>
            </div>
